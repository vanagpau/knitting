...
tab_model(model_LB1, model_LB2, model_LB3, file = "linearmodels.html")
...
df[disc_var] <- lapply(df[disc_var], as.numeric)
...
# note: need to restore factor levels for this - code at LINE 95

# ordinal predictor model
lambda <- c(1000,500,200,100,50,30,20,10,1,0)
model_OFreq1 <- ordSmooth(x = as.numeric(df$Freq_knit) + as.numeric(df$Session_L_Knit), y = df$BDI, lambda = lambda)
summary(model_OFreq1)
...
m2 <- brm(formula = Freq_knit ~ 1 + (1|BDI), data = df, family = (cumulative("probit")), file = "m2")
summary (m2)

m3 <- brm(formula = Freq_knit ~ 1 + (1|BDI) + (1|FMI), data = df, family = (cumulative("probit")), file = "m3")
summary(m3)

loo(m2, m3)

conditional_effects(m2, "Freq_knit", categorical = TRUE)

m4 <- brm(formula = Freq_knit ~ 1 + (1|Participant) + BDI, data = df, family = (cumulative("probit")), file = "m4")
summary(m4)
loo(m2, m3, m4)
