...
disc_var <- c("Freq_knit", "Session_L_Knit", "how_long_knit", "CrotchetQ", "Session_L_Croch")
...
freq_box_fn = function(var1, var2) {
              ggplot(df, aes(x=.data[[var1]], y=.data[[var2]], fill=.data[[var1]])) + geom_boxplot() +  theme(legend.position = "bottom", legend.title = element_blank()) + easy_labs(y = df[,var]) +
  stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, fill = "red") +
    theme(axis.text.x = element_blank(), axis.title.x = element_blank())}

#make boxplots

lapply(c(cont_var, box_fn)
